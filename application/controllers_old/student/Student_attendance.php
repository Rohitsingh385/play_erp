...
class Student_attendance extends MY_Controller {
		...
		?>
		  <tr>
		    ...
		    <td>
			  ...
			  <input type='radio' name='dwa_<?php echo $key; ?>' id='halfday' value='LATE'onclick="hd('LATE',<?php echo $key; ?>)"> <span style='color:orange'><b>LATE</b></span>
			  ...
			  <?php } else{
				...
				<td>
				  ...
				  <input type='radio' name='dwa_<?php echo $key; ?>' id='halfday' value='LATE'onclick="hd('LATE',<?php echo $key; ?>)" <?php if($att_status == 'LATE'){ echo "checked"; }?>> <span style='color:orange'><b>HD</b></span>
				  <?php 
				    if($att_status == 'HD'){
				  ?>
	...
	function att_temp_save(){
		...
		$sec    = $this->input->post('sec');
		$dt     = $this->input->post('dt');
		//$this->db->query("delete from att_temp_save where user_id='$classs'");
		$this->alam->delete('att_temp_save','*');
		$att_date = date('Y-m-d',strtotime($dt));
		
		$stu_data = $this->alam->select('student','ADM_NO,TITLE_NM,FIRST_NM,MIDDLE_NM,ROLL_NO,C_MOBILE,DISP_CLASS',"CLASS='$classs' AND SEC = '$sec' AND Student_Status='ACTIVE' order by ROLL_NO");
			foreach($stu_data as $key => $data){
				$name = $data->FIRST_NM;
				$roll = $data->ROLL_NO;
				$cls = $data->DISP_CLASS;
